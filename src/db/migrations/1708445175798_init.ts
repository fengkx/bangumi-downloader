...
export async function up(db: Db) {
  ...
  await db.schema.createIndex("medias_id_index").on("medias").column("id")
    .execute();
  await db.schema.createIndex("medias_id_file_id_index").on("medias").column(
    "id",
  ).column("file_id").execute();
}
